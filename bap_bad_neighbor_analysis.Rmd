...
There are approximately 6000 non-native species (plant and animal) currently documented in the Lower 48 United States.  Not every species is found or will be found in a given state.  The purpose of this provisional biogeographic analysis package is to create a state-by-state list of regulated non-native (i.e. invasive) species found in adjacent states, but not yet documented in the state of interest.  Stakeholders in states can use the "Bad Neighbor" analysis to see a list of species of concern to develop monitoring and management strategies before invasion.
...
There are a total of `r sum(comb_all_results$bad_neighbor_count)` for the Lower 48 United States.  The table below describes the results by taxon group as total and the relative contribution, or threat, for each group.
...
The chart below illustrates the realitve threat from each of the taxon groups.
...
States that are colored green present a more nuanced result.  Most simply, a lower number of bad neighbors can indicate either a lack of non-native species.  However, a more realistic interpretation is the state is already host to most of the non-native species present in adjacent states.  Florida, for example, is dark green and only has 171 Bad Neighbors, but is known to already be host to many non-native species.
...
# print a text report
cat(generate_state_report_text("Colorado", group_name = "Insect", comb_all_results_trees))
```

The species list is stored as json in the `results_json` directory.  The data were generated on a taxon group-by-group basis, then by state.  The separate taxon group json files need to be combined into a single list for plant groups.  The insect group will be used for illustration since this is only a single taxon group.
...
species_list$species_list %>% 
    ...
    rename("ScientificName" = name, "TSN" = tsn)
```
